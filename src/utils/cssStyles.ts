...
export function bgBlur(props?: { color?: string, blur?: number, opacity?: number }) {
...
export function bgImageBlur(props?: { color?: string, blur?: number, opacity?: number, imgUrl: string }) {
    const color = props?.color ?? '#000000';
    const blur = props?.blur ?? 6;
    const opacity = props?.opacity ?? 0.8;
